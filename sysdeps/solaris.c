 ...
 */
int
kstat_safe_namematch (int num, kstat_t ** ksparg, char *name, void *buf,
                      int size)
{
  kstat_t *ks;
  kstat_t **ksp;
...
int
kstat_safe_retrieve (kstat_t ** ksp, char *module, int instance, char *name,
                     void *buf)
{
  kstat_t *ks;
  kid_t new_kcid;
...
/* dummy */
void
sysdep_set_ioprio (pid_t p, IOPriority ioprio)
{
}

void
sysdep_get_meminfo (ProcessList * this)
{
  long pageSize;
...
/* TODO */
void
sysdep_update_cpu_data (ProcessList * this)
{
  ...
  for (int i = 0; i <= this->cpuCount; i++)
    {
      ...
      ioWait =
        cpu_stat_p->cpu_sysinfo.wait[W_IO] +
...
bool
sysdep_get_process_info (Process * process, pid_t pid)
{
  char filename[34];
  int fd;
  ssize_t r;
